...
describe('Scroll', () => {
  ...
  it('should expect all correct props', () => {
    expect(wrapper.find('Button').props().className)
      .toEqual(' button favorite');
    expect(wrapper.find('Button').props().toggleActive)
      .toEqual(mockFn);
    expect(wrapper.find('Button').props().buttonText)
      .toEqual('View Favorites 2');
    expect(wrapper.find('Button').props().btnFn)
      .toEqual(mockFn);
  });
